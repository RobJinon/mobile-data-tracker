...
function IspList({ onActiveISPChange, ispList, activeISP, editISP }) {
    ...
    return (
        <div>
            ...
            <div className='flex flex-col w-full max-h-screen hidden lg:block'>
                <div className='flex flex-col items-center w-full max-h-screen gap-4'>
                    <div className='text-center w-full bg-primary py-4 text-xl text-white rounded-t-lg font-bold'>My ISPs</div>

                    <div className="flex flex-col items-center gap-4 max-h-full w-full overflow-y-scroll">
                        {ispList.map((isp, index) => (
                            <div className='w-5/6 px-3' key={index}>
                                <div id={isp.ispName} className={`btn group w-full flex hover:justify-between  ${activeISP === isp.ispName ? 'btn-active' :' btn-primary'}`} onClick={()=>handleISPBtn(isp.ispName, isp.id)}>
                                    {isp.ispName}
